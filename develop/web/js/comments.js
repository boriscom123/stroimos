...
(function(window, $, easyXDM){
    ...
    var FOS_COMMENT = {
        ...
        post: function(url, data, success, error, complete) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: data,
                contentType: false, // file upload support
                processData: false, // file upload support
                success: success
        ...
        /**
         * NB: rewritten to support file upload via FormData
         *
        ...
        serializeObject: function(obj)
        {
            ...
            $.each($(obj).find(':input[name], :file[name]'), function (idx, input) {
                var $input = $(input),
                    value = ($input.is(':file') && input.files.length) ? input.files[0] : $input.val();
    ...
    // file upload support
    $(document).on('change', '.comment-file-upload :file', function (e) {
        var $filename = $(e.target).closest('.comment-file-upload').find('.comment-file-upload__filename');

        $filename.val(e.target.value.split(/(\\|\/)/).pop());
    });
