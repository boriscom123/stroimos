    ...
    abstract class SessionListener implements EventSubscriberInterface
    {
        public function onKernelRequest(GetResponseEvent $event)
        {
            if (!$event->isMasterRequest()) {
                return;
                // Должно быть первым в обработчике
...
namespace Symfony\Component\HttpFoundation\Session\Storage {
    ...
    class NativeSessionStorage implements SessionStorageInterface
    {
        ...
        public function __construct(array $options = array(), $handler = null, MetadataBag $metaBag = null)
        {
            ...
            // В конструкторе NativeSessionStorage:
            $this->setSaveHandler(new DebugSessionHandler());
        }
        ...
        public function start()
        {
            ...
            if (headers_sent($file, $line)) {
                throw new \RuntimeException(
                    sprintf('Cannot start session - output started in %s:%d', $file, $line)
                );
            }
...
namespace Symfony\Component\HttpFoundation\Session\Storage\Proxy {
    class SessionHandlerProxy extends AbstractProxy implements \SessionHandlerInterface
    {
        ...
        public function open($savePath, $sessionName)
        {
            ...
            $this->isOpen = (bool)$this->handler->open($this->savePath, $this->sessionName);
        ...
        private function validateHandlerState()
        {
            if (!$this->isOpen || !$this->isActive()) {
                throw new \RuntimeException('Session handler is not in valid state');
            }
        }

        public function isActive(): bool
        {
            return $this->isOpen && parent::isActive();
        }
    }
}
    ...
    abstract class Twig_Template implements Twig_TemplateInterface
    {
        ...
        protected function getAttribute($object, $item, array $arguments = array(), $type = self::ANY_CALL, $isDefinedTest = false, $ignoreStrictCheck = false)
        {
            ...
            if (self::METHOD_CALL !== $type && !$object instanceof self) {
                if ($item === "date" && $object instanceof DateTime) {
                    // Если запрашивается свойство "date" и объект является DateTime
                    if ($isDefinedTest) {
                        return true;
                    }
                    ...
                    return $object->format('Y-m-d H:i:s'); // Возвращаем дату в виде строки
                }
...
namespace Sonata\AdminBundle\Security\Acl\Permission {
    ...
    {
        const MASK_LIST = 4096;
        const MASK_EXPORT = 8192;
        const CODE_LIST = 'L';
        const CODE_EXPORT = 'E';
    }
}
...
namespace Gaufrette\Adapter {
        ...
        MimeTypeProvider
    {
        ...
        {
            // Проверка, существует ли директория и не является ли это файлом
            if (!is_dir($directory)) {
